...
class SkinDiagnosticWindow(ctk.CTkToplevel):
    def __init__(self, parent, model_path):
        ...
        self.lbl_instruct = ctk.CTkLabel(self.panel_left, text="Suba una imagen clara del lunar\no lesión cutánea.", text_color="#747474")
        self.lbl_instruct.pack(pady=5)

        self.btn_load = ctk.CTkButton(self.panel_left, text="📂 CARGAR IMAGEN", command=self.load_image, state="disabled", height=50, fg_color="#67C090", text_color="#FFFFFF", font=ctk.CTkFont(weight="bold"), hover_color="#4E9F75")
        self.btn_load.pack(fill="x", padx=20, pady=30)

        self.lbl_status = ctk.CTkLabel(self.panel_left, text="Inicializando Red Neuronal...", text_color="#F39C12")
        ...
        self.res_frame = ctk.CTkFrame(self.panel_right, fg_color="#D1CFCF", height=100)
        self.res_frame.pack(fill="x", padx=20, pady=(0, 20))
        
        self.lbl_result = ctk.CTkLabel(self.res_frame, text="Esperando análisis...", font=ctk.CTkFont(size=24, weight="bold"), text_color="#0E0E0E")
    ...
    def init_model(self):
        try:
            ...
            self.after(0, lambda: self.lbl_status.configure(text=" Motor IA Calibrado", text_color="#67C090"))
    ...
    def draw_chart(self, val_benign, val_malign):
        ...
        fig.patch.set_facecolor('#D1CFCF')
        ax.set_facecolor('#D1CFCF')

        labels = ['Benigno', 'Maligno']
        sizes = [val_benign, val_malign]
        colors = ['#67C090', '#e74c3c']
        explode = (0, 0.1) if val_malign > val_benign else (0.1, 0) 

        ax.pie(sizes, explode=explode, labels=labels, colors=colors, autopct='%1.1f%%',
               shadow=True, startangle=90, textprops={'color':"#0E0E0E", 'weight':'bold'})
        ax.axis('equal') 
